...
var timeMarcas = false;
function verListaMarcas() {
	clearTimeout(timeMarcas);
	var val = $(this).val().toUpperCase();
	var count = 0;
	$("#listaMarcas").find("i").each(function() {
		var xval = $(this).html().toUpperCase();
		if (xval.indexOf(val) >= 0) {
			$(this).show();
			count++;
		} else {
			$(this).hide();
	...
	if (!count) {
		...
		$("#listaMarcas").css("display", "inline");
	}
}

function ocultaListaMarcas() {
	timeMarcas = setTimeout('$("#listaMarcas").hide()', 1000);
}

function seleccionarMarca() {
	$("input[name='marca']").val($(this).html());
	clearTimeout(timeMarcas);
	$("#listaMarcas").hide();
}